...
TSS2_RC tcti_control_command (
    ...
    uint32_t cmd_code, void *cmd_sdu, size_t cmd_sdu_len,
    uint32_t *resp_code, void *resp_sdu, size_t *resp_sdu_len)
{
    ...
    size_t resp_buf_len = sizeof(ptm_res);
    ...
    resp_buf_len = recv(tcti_swtpm->ctrl_sock, (char *) resp_buf,
                    sizeof(resp_buf), 0);
    if (resp_buf_len < (ssize_t) sizeof(ptm_res)) {
    ...
    if (resp_sdu_len != NULL) {
        *resp_sdu_len = resp_buf_len - sizeof(ptm_res);

        if (resp_sdu != NULL) {
            memcpy(resp_sdu, resp_buf +  sizeof(ptm_res), *resp_sdu_len);
        }
    }
    ...
    const char *conf)
{
    ...
    rc = socket_connect (tcti_swtpm->swtpm_conf.host,
                         tcti_swtpm->swtpm_conf.port,
                         &tcti_swtpm->tpm_sock);
LOG_INFO ("socket_connect returned %d", rc); // TODO
    socket_close (&tcti_swtpm->tpm_sock);
    if (rc != TSS2_RC_SUCCESS) {
    ...
    tcti_swtpm_set_locality(tctiContext, 3);
    if (rc != TSS2_RC_SUCCESS) {
